...
target_retail_dir="/mnt/d/games/World of Warcraft/_retail_/Interface/AddOns"
addon_name="EquipmentManagerClassic"
current_dir=$(pwd)
this_dir=$(dirname $0)
files=("EquipmentManagerClassic.toc" "EquipmentManagerClassic.lua" "embeds.xml")
...
#for file in "${files[@]}"; do cp -v "$current_dir/$file" "$target_retail_dir/$addon_name/."; done

for dir in "${included_dirs[@]}"; do
    if [ -d "$current_dir/$dir" ]; then
        mkdir -p "$target_dir/$addon_name/$dir"
        cp -rv "$current_dir/$dir/." "$target_dir/$addon_name/$dir/."
        #cp -rv "$current_dir/$dir/." "$target_retail_dir/$addon_name/$dir/."
    fi
done
